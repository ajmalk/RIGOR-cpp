...
public:
	...
	// NodeCap stores the unary capacity (.first for src, .second for sink) for a node
	typedef std::pair<tcaptype, tcaptype> NodeCap;
	// SrcSeedNode stores the variable index with the unary capacity for a seed vairable
	typedef std::pair<node_id, NodeCap> SrcSeedNode;
	// SrcSeedList stores multiple seed variables for a graph
	typedef std::vector<SrcSeedNode> SrcSeedList;
	// FGSeedsType stores all seed variables over multiple graphs
	//  FGSeedsType[i][j] accesses the j'th SrcSeedNode for the i'th graph
	//    FGSeedsType[i][j].first would give you the variable index for the seed
	//    FGSeedsType[i][j].second.first would give the seed variable's src unary capacity
	//    FGSeedsType[i][j].second.second would give the seed variable's sink unary capacity
	//  FGSeedsType = std::vector< std::vector< std::pair<int, std::pair<tcaptype, tcaptype> > > >
...
public:
  ...
  static void gassert(bool true_cond, const char* msg);
  static void gassert(bool true_cond, const std::string& msg);
