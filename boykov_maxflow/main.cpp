...
int main(int argc, char* argv[]) {
  ...
  desc.add_options()
      ...
      ("testnesting,n",    po::value<int>(),         "tests parametric min-cut nesting nesting theorem: '1' grid graph; '2' higher order graph");
  ...
  try {
    ...
    if (vm.count("runexample")) {
      ...
      test_seedsolve();
    } else if (vm.count("testnesting")) {
      int graph_type = vm["testnesting"].as<int>();
      test_nesting(graph_type);
    } else {
      std::cerr << desc << std::endl;
    }
  } catch (po::error& e) {
