                         ...
                         const bool& reuse_trees, const bool& rev_lambda)
{
  ...
  for (; (!rev_lambda && lambda_idx < num_params && in_src_cut < num_vars) ||
    ...
    if (reuse_trees) {
      // reuse trees, and run maxflow_reuse_trees_init()
      flow = g->maxflow(true, changed_list, true);
    } else {
      // run normal maxflow init - no tree reusage
      flow = g->maxflow(false, NULL, true);
    }
    flow_vals->push_back(flow);
...
struct ParallelKohliCutComputation : public ParallelCutComputation {
  ...
  ParallelKohliCutComputation(const size_t& num_vars,
      ...
      : ParallelCutComputation(num_vars, num_pairwise_edges, num_params,
        ...
        fg_seed_map(fg_seed_map), reuse_trees(reuse_trees),
                                  ...
                                  const bool& parallel, const bool& print_mode)
{
  ...
  if (print_mode) {
    std::cout << "kohli BK ||=" << parallel << " L=" << num_params << " ";
    int curr_num_seeds;
  ...
  // collect the seed variables for each graph
