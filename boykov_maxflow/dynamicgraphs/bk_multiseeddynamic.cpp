                                ...
                                const bool& rev_lambda)
{
  ...
  unarycaptype flow, s_cap, t_cap;

  // Used to store the prv and current capacities
  unarycaptype* const prv_cap = new unarycaptype[num_vars*2];
  unarycaptype* const prv_cap_t = prv_cap + num_vars;
  unarycaptype* const new_cap = new unarycaptype[num_vars*2];
  unarycaptype* const new_cap_t = new_cap + num_vars;
  ...
  for (; (!rev_lambda && lambda_idx < num_params && in_src_cut < num_vars) ||
    ...
    if (first_iter) {
      ...
      init_min_src_dist(num_vars, distadj_params, graph_type_idx, &(*fg_seeds)[seed_idx], 
                        src_cut_dsts);
      ...
      update_graph_gen_fg_remove_list(g, seed_remove_list, *fg_seeds, *fg_seed_map,
                                      ...
                                      lambda_range, distadj_params, src_cut_dsts, 
                                      graph_type_idx, cuts, new_cap, new_cap_t, 
                                      in_src_cut, precomp_lambda_idx, lambda_idx, 
                                      seed_idx);
      ...
      for (GraphType::node_id var_i = 0; var_i < num_vars; ++var_i) {
        ...
        new_cap[var_i] = s_cap;
        new_cap_t[var_i] = t_cap;
      }
      g->update_capacities(prv_cap, prv_cap_t, new_cap, new_cap_t, false);
    }
    ...
    t0 = tbb::tick_count::now();
    if (first_iter) {
      // thre trees have already been adjusted by transform_seed_trees
      //  hence no need to call maxflow_reuse_trees_init()
      flow = g->maxflow(true, changed_list, false);
    } else {
      // maxflow_reuse_trees_init() only when reusing trees after first iteration
      //  - same as the call in bk_kohli
      flow = g->maxflow(true, changed_list, true);
    }
    ...
    memcpy(prv_cap, new_cap, sizeof(unarycaptype)*num_vars*2);
  ...
  void operator()(const tbb::blocked_range<int>& range) const {
    ...
    for (int seed_idx = range.begin(); seed_idx != range.end(); ++seed_idx) {
      ...
      multiseeddyn_param_maxflow(num_vars, num_pairwise_edges, num_params,
                                 ...
                                 pairwise_edges, distadj_params, cuts+offset,
                                 ...
                                 seed_idx - base_offset_idx, graph_type_idx,
...
void multiseeddyn_param_maxflow_allseeds(const size_t& num_seeds,
                                         ...
                                         const lambdaparamtype* const lambda_range,
                                         const pairwisecaptype* const pairwise_edges,
                                         const DistAdjParams* distadj_params,
                                         const graphtypeidxtype* const graph_type_start_idx,
                                         resulttype* const cuts,
                                         ...
                                         const bool& parallel, const bool& print_mode)
{
  ...
  if (print_mode) {
    std::cout << "multiseed BK ||=" << parallel << " L=" << num_params << " ";
    int curr_num_seeds;
  ...
  ParallelMultiseedDynInit parallelinit(num_seeds, num_vars,
                                        ...
                                        graph_type_start_idx, 
                                        &precomp_lambda_idx, &fg_seeds, 
                                        &fg_seed_map, &gs_precomp);
