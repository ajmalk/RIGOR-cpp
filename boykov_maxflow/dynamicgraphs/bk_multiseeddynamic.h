...
void update_graph_gen_fg_remove_list(GraphType* const g_precomp,
                                     ...
                                     const disttype* const src_cut_dsts,
                                     const size_t& graph_type_idx,
                                     resulttype* const cuts,
                                     unarycaptype* const new_cap_s,
                                     unarycaptype* const new_cap_t,
                                     ...
                                     const int& keep_src_id)
{
  ...
  unarycaptype s_cap_new, t_cap_new;
  unarycaptype s_cap_prv, t_cap_prv;
  ...
  for (GraphType::node_id var_i = 0; var_i < num_vars; ++var_i) {
    ...
    // get the capacity for this node at the current lambda
    get_linear_param_caps(var_i, num_params, nonlambda_s, nonlambda_t,
                          lambda_s, lambda_t, lambda_range,
                          repl_lambda_idx, distadj_params, src_cut_dsts,
                          graph_type_idx, s_cap_new, t_cap_new);
    new_cap_s[var_i] = s_cap_new;
    new_cap_t[var_i] = t_cap_new;
    ...
    if (fg_node_id == keep_src_id) {
      ...
      s_cap_prv = fg_seeds[fg_node_id][fg_src_seed_idx].second.first;
      t_cap_prv = fg_seeds[fg_node_id][fg_src_seed_idx].second.second;
      seed_remove_list[0].push_back(
          GraphType::SrcSeedNode(var_i, GraphType::NodeCap(s_cap_prv, t_cap_prv)));

      seed_remove_list[1].push_back(
          GraphType::SrcSeedNode(var_i, GraphType::NodeCap(s_cap_new, t_cap_new)));
      ...
      get_linear_param_caps(var_i, num_params, orig_nonlambda_s, 
                            orig_nonlambda_t, orig_lambda_s, orig_lambda_t,
                            lambda_range, init_lambda_idx, NULL, NULL, -1, 
                            s_cap_prv, t_cap_prv);
      
      unarycaptype old_cap_diff = s_cap_prv - t_cap_prv;

      /* new unary capacities */
      unarycaptype new_cap_diff = s_cap_new - t_cap_new;
                             ...
                             const int& precomp_lambda_idx)
{
  ...
  for (GraphType::node_id var_i = 0; var_i < num_vars; ++var_i) {
    ...
    if (fg_node_id == INVALID_SRC_ID) {
      ...
      get_linear_param_caps(var_i, num_params, nonlambda_s, nonlambda_t,
                            lambda_s, lambda_t, lambda_range,
                            precomp_lambda_idx, NULL, NULL, -1, s_cap, t_cap);
